        ...
        <div class="flex gap-3">
            @if(!$isHistory && $colocation->status === 'active')
                ...
                <div x-data="{ openExpense: false }">
                    ...
                    <div x-show="openExpense" class="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-4" x-transition x-cloak>
                            ...
                            <form action="{{ route('expenses.store', $colocation->id) }}" method="POST">
                                ...
                                <div class="space-y-4">
                                    <div>
                                        ...
                                        <input type="text" name="title" required class="w-full bg-slate-50 border-none rounded-xl focus:ring-2 focus:ring-indigo-500">
                                    </div>
                                    <div class="grid grid-cols-2 gap-4">
                                        <div>
                                            <label class="block text-[10px] font-black uppercase text-slate-400 mb-1">Montant (€)</label>
                                            <input type="number" step="0.01" name="amount" required class="w-full bg-slate-50 border-none rounded-xl focus:ring-2 focus:ring-indigo-500">
                                        </div>
                                        <div>
                                            <label class="block text-[10px] font-black uppercase text-slate-400 mb-1">Date</label>
                                            <input type="date" name="date" value="{{ date('Y-m-d') }}" required class="w-full bg-slate-50 border-none rounded-xl focus:ring-2 focus:ring-indigo-500">
                                        </div>
                                    </div>
                                    <div>
                                        <label class="block text-[10px] font-black uppercase text-slate-400 mb-1">Catégorie</label>
                                        <select name="category_id" required class="w-full bg-slate-50 border-none rounded-xl focus:ring-2 focus:ring-indigo-500">
                                ...
                                <div class="flex gap-3 mt-6">
                                    <button type="button" @click="openExpense = false" class="flex-1 py-3 bg-slate-100 rounded-xl font-bold hover:bg-slate-200 transition-colors">Annuler</button>
                                    <button type="submit" class="flex-1 py-3 bg-indigo-600 text-white rounded-xl font-bold hover:bg-indigo-700 transition-colors">Enregistrer</button>
                                </div>
                            </form>
                        </div>
                    </div>
    ...
    <div class="grid grid-cols-1 lg:grid-cols-4 gap-8">
        ...
        <div class="lg:col-span-1 space-y-6">
            <div class="bg-white p-6 rounded-3xl border border-gray-100 shadow-sm">
                <h4 class="font-black mb-6 text-gray-800 text-xs uppercase tracking-widest text-center lg:text-left">Résumé des Soldes</h4>
                <div class="space-y-4">
                        ...
                        <div class="flex items-center justify-between p-2 {{ auth()->id() == $userId ? 'bg-indigo-50 rounded-xl' : '' }}">
                            <div class="flex flex-col">
                                <span class="text-sm font-bold {{ $data['is_left'] ? 'text-gray-400 line-through' : 'text-slate-700' }}">
                                    {{ $data['user']->name }}
                                    @if(auth()->id() == $userId)
                                        <span class="ml-1 text-[10px] bg-indigo-200 text-indigo-700 px-1.5 py-0.5 rounded uppercase font-black tracking-tighter">Moi</span>
                                    @endif
                                </span>
                ...
                @if($colocation->expenses->isEmpty())
                    <div class="py-20 text-center">
                        <div class="inline-flex items-center justify-center w-16 h-16 bg-slate-50 rounded-full mb-4">
                            <svg class="w-8 h-8 text-slate-300" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" stroke-width="2" stroke-linecap="round"></path></svg>
                        </div>
                        <p class="text-slate-400 font-medium">Aucune dépense enregistrée pour le moment.</p>
                    </div>
                @else
                        ...
                        @foreach($colocation->expenses()->orderBy('date', 'desc')->get() as $expense)
                            <div class="p-6 flex justify-between items-center hover:bg-slate-50 transition-all group">
                                <div class="flex items-center gap-4">
                                    <div class="h-12 w-12 bg-indigo-50 text-indigo-600 rounded-2xl flex items-center justify-center font-black group-hover:bg-indigo-600 group-hover:text-white transition-colors shadow-sm">
                                        {{ strtoupper(substr($expense->title, 0, 1)) }}
                                    </div>
                                    <div>
                                        <p class="font-bold text-slate-900 leading-tight">{{ $expense->title }}</p>
                                        <p class="text-xs text-slate-400 mt-1">
                                            Payé par <span class="text-indigo-600 font-semibold">{{ $expense->payer->name }}</span> • {{ \Carbon\Carbon::parse($expense->date)->format('d/m/Y') }}
                                        </p>
                                    </div>
                                </div>
                                <div class="text-right">
                                    <p class="font-black text-slate-900 text-lg">{{ number_format($expense->amount, 2) }} €</p>
                                    @if($expense->category)
                                        <span class="text-[10px] uppercase font-bold text-slate-300">{{ $expense->category->name }}</span>
                                    @endif
                                </div>
                            </div>
                        @endforeach
